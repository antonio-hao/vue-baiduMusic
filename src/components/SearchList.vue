<template>
  <div class="searchlist">
    <div class="panel hotsongs on" v-loading.fullscreen.lock="fullscreenLoading">
      <ul class="list">
        <router-link :key="index" :to="{name:'MusicPlay',params:{songid:item.songid}}" tag="li" class="song" v-for="(item,index) in searchData.song">
          <div class="info">
...
<script>
    export default {
      ...
      created(){
        const searchtUrl = this.HOST + "/v1/restserver/ting?method=baidu.ting.search.catalogSug&query=" + this.content;
        this.$axios.get(searchtUrl)
          .then(res=>{
